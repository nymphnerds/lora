...
echo "This can take a while on first install. Large Hugging Face files may sit on one item for several minutes."
...
def repo_summary(repo_id: str, patterns: list[str]) -> None:
    try:
        info = HfApi().model_info(repo_id, files_metadata=True)
    except Exception as exc:
        print(f"Hugging Face metadata check skipped: {exc}", flush=True)
        return
    ...
    total = sum(int(getattr(sibling, "size", 0) or 0) for sibling in selected)
    print(
        f"Hugging Face bundle plan: {repo_id} -> {len(selected)} files, about {format_bytes(total)}.",
        flush=True,
    )


def start_monitor(root: Path, label: str) -> tuple[threading.Event, threading.Thread]:
    ...
    def monitor() -> None:
        while not stop.wait(20):
            ...
            print(
                f"{label} still downloading: elapsed={elapsed}s local_files={count} local_size={format_bytes(total)}",
                flush=True,
            )
...
repo_summary("Tongyi-MAI/Z-Image-Turbo", allow_patterns)
before_count, before_size = local_summary(model_root)
print(
    f"Local model cache before download: {before_count} files, {format_bytes(before_size)} at {model_root}",
    flush=True,
)
monitor_stop, monitor_thread = start_monitor(model_root, "Z-Image Turbo model bundle")
...
after_count, after_size = local_summary(model_root)
print(
    f"Local model cache after download: {after_count} files, {format_bytes(after_size)} at {model_root}",
    flush=True,
)
...
echo "Checking/downloading the assistant LoRA adapter weights..."
...
def repo_summary(repo_id: str, patterns: list[str]) -> None:
    try:
        ...
        print(f"Hugging Face adapter metadata check skipped: {exc}", flush=True)
        return
    ...
    total = sum(int(getattr(sibling, "size", 0) or 0) for sibling in selected)
    print(
        f"Hugging Face adapter plan: {repo_id} -> {len(selected)} files, about {format_bytes(total)}.",
        flush=True,
    )
...
repo_summary("ostris/zimage_turbo_training_adapter", allow_patterns)
before_count, before_size = local_summary(adapter_dir)
print(
    f"Local adapter cache before download: {before_count} files, {format_bytes(before_size)} at {adapter_dir}",
    flush=True,
)
monitor_stop, monitor_thread = start_monitor(adapter_dir, "Turbo training adapter")
...
after_count, after_size = local_summary(adapter_dir)
print(
    f"Local adapter cache after download: {after_count} files, {format_bytes(after_size)} at {adapter_dir}",
    flush=True,
)
